...
StatusBar.prototype.renderLives = function () {
	...
	var sx = this.heartSprite.extents.minx,
		sy = this.heartSprite.extents.miny,
		sw = this.heartSprite.extents.maxx - this.heartSprite.extents.minx,
		sh = this.heartSprite.extents.maxy - this.heartSprite.extents.miny,
		life,
		dx,
		dy,
		dw,
		dh;
	// for each life...
	for (life = 0; life < player.lives; life++) {
		//calculate where the heart should end up on the status bar:
		dx = 5 + ((life % 7) * (this.heartSprite.extents.maxx - this.heartSprite.extents.minx + 5)) / 3;
		dy = 5
			+ (Math.floor(life / 7) * (this.heartSprite.extents.maxy - this.heartSprite.extents.miny + 5)) / 3;
		dw = (this.heartSprite.extents.maxx - this.heartSprite.extents.minx) / 3;
		dh = (this.heartSprite.extents.maxy - this.heartSprite.extents.miny) / 3;
