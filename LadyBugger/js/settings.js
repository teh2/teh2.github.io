...
Settings.prototype.init = function () {
	...
	this.curCharIndex = 0;
	for (charIndex = 0; charIndex < this.chars.length; charIndex++) {
...
Settings.prototype.render = function () {
	...
	img = Resources.get(this.chars[this.curCharIndex]);
...
Settings.prototype.handleInput = function (keyCode) {
	...
	if ('left' === theKey) {
		...
		if (0 < this.curCharIndex) {
			this.curCharIndex--;
		} else {
			this.curCharIndex = this.chars.length - 1;
		};
		this.render();
		return true;
	} else if ('right' === theKey) {
		//show next character sprite
		if (this.curCharIndex < this.chars.length - 1) {
			this.curCharIndex++;
		} else {
			this.curCharIndex = 0;
		};
